<!-- Pockets — universal canvas for PocketPaw dashboards.
     Created: 2026-03-21. Replaces /command-center as the primary Pockets route.
     Each Pocket is a kit (YAML layout + data sources + workflows).
     Mission Control, Deep Work, Sprint Tracker, etc. are all Pocket types.
-->
<script lang="ts">
...
<div class="flex h-full flex-col overflow-auto">
  ...
  {:else}
    ...
    <div class="flex h-full flex-col items-center justify-center gap-5 p-8 text-center">
      ...
      <div>
        <h2 class="text-lg font-semibold text-foreground">Welcome to Pockets</h2>
        <p class="mt-1 max-w-sm text-sm text-muted-foreground">
          Pockets are living dashboards powered by your agent. Install one to manage tasks, track sprints, organize research, or run content pipelines.
        </p>
      </div>
      <button
